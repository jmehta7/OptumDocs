...
-- Jatin Mehta : 05/20/2019 : To restrict the data of table dbo.Temp_** for three years processing i.e current +- 1  
...
and  YEAR(g.EffectiveDate) IN (SELECT Years FROM dbo.udfGetPrevCurrNextYears()) -- limit processing for three years
...
DELETE FROM [dbo].[Temp_GroupBenSys] where YEAR(GroupEffDate) IN (SELECT [Years] FROM dbo.udfGetPrevCurrNextYears())
...
On g.GroupID = g38.GroupID and g38.BenefitTypeID = 38 --and g.beneffdate = g38.beneffdate -- g.RowID = g38.RowID and g38.BenefitTypeID = 38    
 ...
 Where YEAR(g.GroupEffDate) IN (SELECT [Years] FROM dbo.udfGetPrevCurrNextYears()) -- Limit processing for three years
