...
-- Jatin Mehta : 03/13/2019 : Removed index of dbo.Temp_GroupBenSys table , added insert into statement , change from DROP to DELETE for Attunity requirement   
-- Jatin Mehta : 05/20/2019 : To restrict the data of table dbo.Temp_GroupBenefit for three years i.e current +- 1  
...
inner join [Group] gp 
on egpb.GroupID = gp.GroupID
 Where YEAR(gp.EffectiveDate) IN (SELECT Years FROM dbo.udfGetPrevCurrNextYears())   -- restrict 3 year processing
 
 and egpb.GroupID Not In (Select GroupID from #GroupBenefit)
--and YEAR(gpb.EffectiveDate) IN (SELECT Years FROM dbo.udfGetPrevCurrNextYears())       
...
where YEAR(g.EffectiveDate) IN (SELECT Years FROM dbo.udfGetPrevCurrNextYears())       
