...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Temp_HospitalCopay_IDCardElements]') AND type in (N'U'))
  ...
  Where BenefitID IN (
  Select BenefitID from #HospitalCopayIDCardElement thc  inner join [Group] gp ON thc.GroupID = gp.GroupID
...
(bd2.CopayMinAmount as varchar) + ' ' + '-' + ' ' + Cast(bd2.CopayMaxAmount as varchar))           
 ...
 User as CreateBy,              
  Getdate() as CreateDate              
        ...
        --      SELECT  GroupID,PlanID,BenefitId FROM [EmployerGroupPlanBenefit] [EGPlan] WITH(NOLOCK)              
        --    )[GPlan]
		   INNER JOIN (SELECT GroupID,PlanID,BenefitId FROM #HospitalCopayIDCardElement [IGPlan]  
        )[GPlan]
