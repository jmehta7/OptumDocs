...
ALTER Proc [dbo].[RPT_iGroupBenefits]    
As    
Begin   
...
Where gp.EffectiveDate <> IsNull(gp.TerminationDate,'9999-12-31') 
and YEAR(gp.EffectiveDate) IN (SELECT Years FROM dbo.udfGetPrevCurrNextYears())   -- restrict 3 year processing
...
DELETE FROM TGB
From [dbo].[Temp_GroupBenefit] TGB
Inner Join [Group] GP WITH(NOLOCK) ON GP.GroupID = TGB.GroupID
where  GP.EffectiveDate <> IsNull(GP.TerminationDate,'9999-12-31')   
and YEAR(GP.EffectiveDate ) IN (SELECT Years FROM dbo.udfGetPrevCurrNextYears()) 
