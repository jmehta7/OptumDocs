/* Modified Date      : 10/28/2015          
   ...
   */    
   -- Jatin Mehta : 05/20/2019 : To restrict the data of table dbo.[Temp_HospitalCopay_IDCardElements] for three years i.e current +- 1  
       
ALTER Proc [dbo].[RPT_iHospitalCopay_IDCardElements]              
AS              
		 SELECT [IGPlan].GroupID,[IGPlan].PlanID,ipb.BenefitId 
		 into dbo.#HospitalCopayIDCardElement
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Temp_HospitalCopay_IDCardElements]') AND type in (N'U'))              
  ...
  Where BenefitID IN (
  Select BenefitID from dbo.#HospitalCopayIDCardElement thc  inner join [Group] gp  WITH(NOLOCK)ON thc.GroupID = gp.GroupID
...
(bd1.CopayMinAmount as varchar) + ' ' + '-' + ' ' + Cast(bd1.CopayMaxAmount as varchar))           
 ...
 (Cast(bdt1_Tier1.CoPayMinAmount AS varchar) + ' ' + '-' + ' ' + CAST(bdt1_Tier1.CoPayMaxAmount AS varchar)+ ' '+ '/' + ' ' + CAST(bd1.copayMinamount as varchar))            
  ...
  WHEN bdt1_Tier2.TierID IS NULL AND bdt1_Tier1.TierID IS NOT NULL           
     ...
     AND bd1.CopayMinAmount IS NOT NULL          
     THEN             
 (Cast(bdt1_Tier1.CoPayMinAmount AS varchar)+ ' '+ '/' + ' ' + CAST(bd1.copayMinamount as varchar)  + ' ' + '-' + ' '+ CAST(bd1.CopayMaxAmount as varchar))            
...
Cast(bd2.CoInsuranceMaxPercent as varchar))           
 ...
 User as CreateBy,              
  Getdate() as CreateDate              
        ...
        INNER JOIN [PlanType] [PT] WITH(NOLOCK)               
		   ...
		   INNER JOIN dbo.#HospitalCopayIDCardElement [GPlan]
 ...
 Drop table dbo.#HospitalCopayIDCardElement
